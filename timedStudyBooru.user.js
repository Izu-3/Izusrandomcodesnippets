...
console.log(fullTimer);
    ...
    $('.showHistory').click(function(){$('.historyContainer').toggleClass('historyVisible');
                                       console.log($('.historyVisible').length);
...
$(window).on("load", function() {
    if (enabled==true) {
       if(countupMode){$('.studymode').prop('disabled',false); countupTimer = setInterval(countup, 1000);}
       if(!countupMode){$('.studymode').prop('disabled',false); countdownTimer = setInterval(countdown, 1000);}
    }
});
...
function startStudying(){
    ...
    $('.studyContainer').toggleClass('timerRunning');
    if(enabled==true){
...
function addHistory(){
    ...
    history.push([window.location.href,$(imgContainer).attr('data-file-url')]);
}
...
function countdown(){
    if(enabled){
        ...
        if(timeout < 1){
            ...
            if(setLength!=0){setTimeout(changeImage(false), (restDelay*1000));}
                       }
...
function addStudyOption(studyName,isEnabled){
     $('.searchTermOptionContainer').append("<div class='searchTermOption'><input type='text' class='studyTopicOption' value='"+studyName+"' enabled='"+isEnabled+"'></input><div class='removeInput' onclick='$(this).parent().remove();'><p>-</p></div><div class='disableInput' onclick=\"let setting = $(this).parent().children('.studyTopicOption').attr('enabled') == 'true';;$(this).parent().children('.studyTopicOption').attr('enabled',!setting)\"><p>=</p></div></div>")
}
...
<div class='searchTermAddOption'><p>Add New</p></div><div class='searchTermSave'><p>Save List</p></div></div>\
...
.studyModeActive input,.studyModeActive textarea,.studyModeActive select{color:#fff!important;}.studyModeActive #image-container{position:fixed;top:0;left:0;display:flex;width:100vw;height:100vh;background:black;justify-content:center;z-index:2;}\
